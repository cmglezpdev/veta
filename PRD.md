...
- Ship as an installable/runnable CLI (`npx`, global install, or standalone binary) whose only external runtime dependency — the `yt-dlp` binary — is obtained automatically at install time, never by manual user setup.
...
| FR-24 | The CLI resolves the `yt-dlp` binary by preferring a user-managed installation found on `PATH`, falling back to the bundled binary obtained at install time. The resolved source and version are discoverable by the user. |
...
| ID | Requirement |
|----|-------------|
| NFR-1 | **Exactly one external runtime dependency: the `yt-dlp` binary**, obtained automatically at install time via `youtube-dl-exec` rather than requiring a manual user install. No Python runtime is required (yt-dlp ships as a self-contained executable). If the binary is missing or unusable, the CLI fails with actionable install instructions rather than an opaque error. *(Revised: the original "zero external dependencies" goal proved unattainable — see §9.)* |
